...
const CityModalize = ({modalizeRef, setBottomSheetVisible, data, tempVehicleState, setTempVehicleState}) => {
                        ...
                        renderItem={({item}) => (
                            <TouchableOpacity style={{ paddingVertical: 10, paddingHorizontal:5 }}
                                onPress={() => {
                                    setTempVehicleState({
                                        ...tempVehicleState,
                                        fromPlace: item?.stateGstCode,
                                        fromPlaceName: item?.name
                                    })
                                    setFilterStates([]);
                                    setSearchedText("");
