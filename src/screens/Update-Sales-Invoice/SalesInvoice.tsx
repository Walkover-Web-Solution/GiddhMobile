...
export class SalesInvoice extends React.Component<Props, State> {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      // ----- States for handling Update Voucher -----
      voucherUniqueName: this.props.route?.params?.voucherUniqueName ?? '',
      allStockVariants: {}
    };
  ...
  componentDidUpdate(prevProps: Readonly<Props>, prevState: Readonly<State>) {
    ...
    // console.log('---===---', prevProps?.route?.params?.voucherUniqueName, '\n', this.props?.route?.params?.voucherUniqueName, '\n', 
    // this.state.voucherUniqueName, prevState.voucherUniqueName
    // )
    if (prevProps?.route?.params?.voucherUniqueName !== this.props?.route?.params?.voucherUniqueName) {
      // this.setState({ voucherUniqueName: this.props?.route?.params?.voucherUniqueName ?? '' }, 
      // () => {
      //   this.getPartyDataForUpdateVoucher(this.props?.route?.params?.voucherUniqueName)
      // })
  ...
  async mapEntriesToUIData (entries: Array<any>) {
    ...
    await Promise.all(entries.map(async (entry) => {
      ...
      // let particularData : any = {};
      // const results = await InvoiceService.getSalesDetails(entry.transactions[0].account.uniqueName);
      // if (results && results.body) {
      //     particularData = results.body;
      //     particularData["newUniqueName"] = particularData.uniqueName + Math.floor(Math.random() * 1000).toString().padStart(3, '0'); // Used to identify and Edit multiple same entries
      // }
      ...
      const modifiedEntryObj = {
        // "name": entry.transactions[0].account.name,
        // "uniqueName": entry.transactions[0].account.uniqueName,
        // "currency":{
        //   "code":"INR",
        //   "symbol":"₹"
        // },
        // "applicableDiscounts": entry?.discounts,
        // "applicableTaxes":[
        ...
        "discountValue": entry?.discounts[0]?.discountValue ?? 0,
        "isNew": false,
        "description": entry?.description,
        "unit": isStock ? entry.transactions[0].stock.quantity : '',
        "total": entry?.subTotal?.amountForAccount,
        "discountPercentage": 0,
        "discountType": entry?.discounts[0]?.calculationMethod,
        // "taxText": entry?.taxTotal?.amountForAccount,
        "taxType": 0,
    ...
    // editItemDetails.isNew = false
      ...
      console.log('----- modifiedEntryObj -----', JSON.stringify(modifiedEntryObj))
  ...
  mapAddressFromVoucherData(setCustomAddress: boolean, addressArray, voucherBillingDetails: any, voucherShippingDetails: any): any {
    let partyBillingAddress : any = {
      address: '',
      gstNumber: '',
      state: {
        code: '',
    ...
    let partyShippingAddress: any = { ...partyBillingAddress }
    ...
    if(setCustomAddress){
      ...
      console.log('{{{ partyBillingAddress }}}}',partyBillingAddress)
    } else {
      partyBillingAddress = addressArray.find(obj => obj.address == voucherBillingDetails?.address[0]);
      console.log('{{{--- partyBillingAddress ---}}}}',partyBillingAddress)
      partyShippingAddress = addressArray.find(obj => obj.address == voucherShippingDetails?.address[0]);
    }

    return { partyBillingAddress, partyShippingAddress }
  }
    ...
    try {
      ...
      if(!this.props.route?.params.isSalesCashInvoice){
        ...
        if (results.body && results.body.results) {
          ...
          console.log('---[[[ BANDE KA DATA ]]]---', JSON.stringify(accountData));
          this.setState({
      ...
      console.log('-----+++++++++++++++++++------', response.body.voucherTotal.amountForAccount, response.body.balanceTotal.amountForAccount)

      if(response?.status === 'success'){

        const { partyBillingAddress, partyShippingAddress } = this.mapAddressFromVoucherData(!!this.props.route?.params.isSalesCashInvoice, addressArray, response?.body?.account?.billingDetails, response?.body?.account?.shippingDetails); 

        console.log(')))))))', partyBillingAddress, partyShippingAddress)
        ...
        this.setState({
          ...
          partyBillingAddress,
          partyShippingAddress,
          billSameAsShip: partyBillingAddress.address == partyShippingAddress.address,
          addressArray
        })
  ...
  async updateVoucher() {
    ...
    try {
      ...
      if(response?.status === 'success') {
        ...
        Toast({ message: 'Voucher updated successfully!' });
        this.props.navigation.goBack();
      }
    } catch (error) {
  ...
  renderStockItem(item) {
    // console.log('-------- SWIPEABLE ---------', JSON.stringify(item));
    return (
      <Swipeable
  ...
  render() {
    // console.log('-------------', !!this.state.dueDate, this.props.route?.params)
    return (
      <View style={{ flex: 1 }}>
