  ...
  _variantsBottomSheet (){
    const renderItem = ({item}) => {
      ...
      return (
        ...
        >
          { index !== -1
            ?  <AntDesign name="checksquare" size={20} color={"#3399ff"} />
  ...
  async searchUser() {
    ...
    try {
      const results = await InvoiceService.search(
        ...
        1,
        this.props.companyVoucherVersion == 1 ? 'otherincome%2C%20revenuefromoperations' : 'otherincome%2C%20revenuefromoperations%2C%20fixedassets',
        true
      );
      ...
      this.setState({ itemList: [], searchError: this.props.t('common.noResultsFound'), isSearchingParty: false });
    }
  }
    ...
    try {
      if (item.stock) {
        ...
        if (results && results.body) {
          ...
          this.setState({ searchError: this.props.t('common.noResultsFound'), loading: false });
        }
      } else {
    ...
    return (
      <Animated.View style={[style.footerAddItemConatiner, { marginBottom: this.keyboardMargin }]}>
        ...
        <TouchableOpacity
          onPress={() => {
            this.props.navigation.goBack();
            DeviceEventEmitter.emit(APP_EVENTS.updateItemInCreditNote, this.state.addedItems);
          }}
          style={{
            height: 32,
            borderRadius: 16,
          ...
          <Text style={style.addItemDone}>{this.props.t('common.done')}</Text>
        </TouchableOpacity>
      </Animated.View>
    );
  }
...
const ConnectedAddItemScreen = connect(mapStateToProps, mapDispatchToProps)(AddItemScreen);

const Screen = (props) => {
  const { t } = useTranslation();
  return <ConnectedAddItemScreen {...props} t={t} />;
};
