...
export class SalesInvoice extends React.Component<Props> {
      ...
      this.setState({
        ...
        pdfPreviewParams: {
          ...
          voucherInfo: {
            ...
            voucherType: INVOICE_TYPE.credit
          }
        }
      });
    }, 300);
  ...
  copyVoucherFromList = async (item) => {
    ...
    try {
      ...
      Alert.alert(this.props.t('common.alert'), e?.data?.message ?? 'Error while copying invoice');
    } finally {
  ...
  componentDidMount() {
    ...
    this.listener = DeviceEventEmitter.addListener(APP_EVENTS.updatedItemInInvoice, (data) => {
      console.log('data----------> updatedItemInInvoice', data);
      ...
      {this.state.invoiceType !== INVOICE_TYPE.cash ? (
        <View
          ...
          onPress={() => this.openCopyVoucherSheet()}>
            <FontAwesome5 name={'copy'} color={'#229F5F'} style={{ marginLeft: 16 }} size={18} />
            <Text style={{ color: '#1C1C1C', marginLeft: 10, fontFamily: 'AvenirLTStd-Book' }}>{this.props.t('common.Copy Previous Sales Invoices')}</Text>
          </TouchableOpacity>
        </View>
      ) : null}
      </>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              <Text style={style.searchItemText}>{item.name ? item.name : this.props.t('common.noResultsFound')}</Text>
            </TouchableOpacity>
          )}
        />
        <TouchableOpacity
  ...
  updateAddedItems = async (addedItems) => {
    ...
    console.log("updateAmountToCurrentCurrency[i]------------>", updateAmountToCurrentCurrency);
    for (let i = 0; i < updateAmountToCurrentCurrency.length; i++) {
    ...
    try {
      if (Platform.OS == "android") {
        ...
        if(granted !== PermissionsAndroid.RESULTS.GRANTED){
            ...
            Alert.alert(this.props.t('purchaseBill.permissionDenied'), this.props.t('purchaseBill.storagePermissionMessage'));
            return;
          }
        }
      }
    ...
    const renderFooter = () => {
      return (
        <TouchableOpacity style={{ backgroundColor: '#229F5F', width: "30%", margin: 10, padding: 5, paddingVertical: 10, borderRadius: 10, alignItems: "center", alignSelf: "flex-end" }}
          ...
          <Text style={[style.boldText, {color: '#FFFFFF'}]}>{this.props.t('addItemScreen.done')}</Text>
        </TouchableOpacity>
      )
    }
        return (
    ...
    return (
      <View style={{ paddingVertical: 30, alignItems: 'center' }}>
        {this.state.fetchingCopyVouchers ? (
          <ActivityIndicator color={'#229F5F'} size="small" />
        ) : (
  ...
  _renderCopyVoucherSheet() {
    return (
      <BottomSheet
        ...
        headerText={this.props.t('common.Copy Previous Sales Invoices')}
