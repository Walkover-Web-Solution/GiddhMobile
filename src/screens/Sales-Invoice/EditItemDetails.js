...
class EditItemDetails extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      selectedArrayType: sanitizedTaxDetails.map((r) => r && r.taxType).filter(Boolean),
  ...
  /**
   * Full company tax list for the bottom sheet so every tax can be selected or cleared.
   * Line tax totals use getTaxRowsForCalculation + calculatedTaxAmount (same as SalesInvoice).
   */
  getTaxPickerRows() {
    const taxArr = this.props.taxArray || [];
    return taxArr.slice();
  ...
  componentDidMount() {
    ...
    this.setState(
      {
        editItemDetails,
        selectedArrayType: typesFromRows,
      },
      () => {
            ...
            return (
              <TouchableOpacity
                  ...
                  if (
                    ...
                    if (filtered.length == 0) {
                      ...
                      if(item.taxType == 'tdspay' || item.taxType == 'tcspay' || item.taxType == 'tdsrc' || item.taxType == 'tcsrc'){
                        // await this.calculatedTaxAmount(itemDetails)
                      ...
                      this.setState({ editItemDetails: itemDetails, selectedArrayType: arr1 });
                    } else {
                      var filtered = _.filter(selectedTaxArray, function (o) {
                        if (o.uniqueName !== item.uniqueName) {
                          return o;
                      ...
                      const total = this.calculateFinalAmount(itemDetails);
                      itemDetails.total = total;
                      this.setState({ editItemDetails: itemDetails, selectedArrayType: arr2 });
                    }
                    this.calculateFinalTcsOrTdsToDisplay();
                  }
                }}>
                ...
                <TouchableOpacity style={style.tdsTcsCalculationButton} onPress={() => this.setBottomSheetVisible(this.taxCalculationModalRef, true)}>
                  <Text style={style.tdsTcsCalculationButtonText}>{this.state.editItemDetails.tdsTcsTaxCalculationMethod == 'OnTaxableAmount' ? this.props.t('purchaseItemEdit.onTaxableValueShort') : this.props.t('purchaseItemEdit.onTotalValueShort')}</Text>
  ...
  buildMergedLineForTax(editItemDetails) {
    const base = this.props.itemDetails || {};
    const e = editItemDetails || {};
    const discountValue = this.calculateDiscountedAmountToDisplayTotalAmount(e);
    ...
    return {
      ...
      rate: rateRaw,
      discountValue,
      taxDetailsArray: e.taxDetailsArray || [],
    };
  }
  ...
  getTaxableAmountForItem(itemDetails) {
    ...
    const disc = Number(itemDetails.discountValue != null ? itemDetails.discountValue : 0);
    const d = Number.isFinite(disc) ? disc : 0;
    return Math.max(0, base - d);
  }

  /**
   * Same rules as SalesInvoice.calculatedTaxAmount (getTaxRowsForCalculation + SEZ/INR branches).
   * @param {string} calculateFor 'taxAmount' | 'totalAmount' | 'InvoiceDue'
   */
  calculatedTaxAmount(itemDetails, calculateFor = 'taxAmount') {
    if (calculateFor === 'totalAmount' && this.props.zeroLineTotalTax) {
      return 0;
    }
    const merged = this.buildMergedLineForTax(itemDetails);
    let amt = this.getTaxableAmountForItem(merged);
    const rows = this.getTaxRowsForCalculation(merged);
    let totalTax = 0;
    const useInrSez = !!this.props.useInrSezTaxRowLogic;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (useInrSez) {
        ...
        if (
          ...
          totalTax = row.taxType == 'tdspay' || row.taxType == 'tdsrc' ? totalTax - taxAmount : totalTax + taxAmount;
  ...
  calculateFinalAmount(editItemDetails) {
    ...
    const totalTax = this.calculatedTaxAmount(editItemDetails, 'totalAmount');
  ...
  calculatedTaxAmountToDisplayTotalAmount(itemDetails) {
    return this.calculatedTaxAmount(itemDetails, 'taxAmount');
  }
  ...
  */
      ...
      <View
        ...
        <View>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginLeft: 16, marginTop: 10 }}>
            ...
            <Text style={{ marginLeft: 10 }}>{this.props.t('otherInfo.hsnCode')}</Text>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginLeft: 16, marginTop: 15 }}>
            ...
            <Text style={{ marginLeft: 10 }}>{this.props.t('otherInfo.sacCode')}</Text>
          </View>
        </View>
        <TextInput
  ...
  _renderDescriptionField() {
    return (
      <TextInput
        placeholder={this.props.t('common.addDescription')}
        multiline={true}
        numberOfLines={10}
      ...
      <View style={{ flex: 1, backgroundColor: 'white' }}>
        ...
        {this.props.itemDetails.stock &&
          this._renderTwoFieldsTextInput(
            ...
            this.props.t('purchaseItemEdit.unit'),
        ...
        {this._renderTwoFieldsTextInput(
          ...
          this.props.t('purchaseItemEdit.amount'),
        ...
        <TouchableOpacity
          onPress={() => {
            ...
            const selectedUniqueNames = (editItemDetails.taxDetailsArray || [])
              .map((t) => (t && t.uniqueName ? t.uniqueName : null))
              .filter(Boolean);
            const selectedTypesFromRows = (editItemDetails.taxDetailsArray || [])
              .map((t) => t && t.taxType)
              .filter(Boolean);
            if (editItemDetails.item.stock) {
              editItemDetails.item.stock.taxes = selectedUniqueNames;
            } else {
        ...
        <TouchableOpacity
          ...
          {this.state.editItemDetails.percentDiscountArray.length > 0 ? (
            <Text style={style.TaxText} numberOfLines={2}>
              ...
              <Text style={style.bottomSheetSelectTaxText}>{this.props.t('editItemDetails.selectDiscount')}</Text>
            )}
    ...
    return (
      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        <View style={{ marginHorizontal: 16, flex: 1, paddingVertical: 10 }}>
          <TouchableOpacity
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Text style={{ marginLeft: 10, color: this.props.taxArray.length > 0 ? 'black' : '#808080' }}>{this.props.t('product.tax')}</Text>
            </View>
        ...
        <View style={{ marginHorizontal: 16, flex: 1, paddingTop: 16, paddingBottom: 8 }}>
          <Text style={[style.itemFieldValue, { paddingTop: 16 }]}>{this.state.editItemDetails.taxText.toFixed(2)}</Text>
...
function EditItemDetailsWrapper(props) {
  const { t } = useTranslation();
  return <EditItemDetails {...props} t={t} />;
}

export default EditItemDetailsWrapper;
