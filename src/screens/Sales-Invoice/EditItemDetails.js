...
class EditItemDetails extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      editItemDetails: {
        ...
        discountPercentageText: this.props.itemDetails.discountPercentage ? this.props.itemDetails.discountPercentage : '',
  ...
  _renderTax() {
    ...
    return (
      <View style={{ backgroundColor: 'transparent', width: '100%', height: '100%', position: 'absolute', justifyContent: 'center', alignItems: 'center' }}>
        <TouchableOpacity style={{ backgroundColor: 'black', opacity: 0.5, width: '100%', height: '100%', position: 'absolute' }} onPress={() => { this.setState({ showTaxPopup: false }) }} />
        <View style={{ backgroundColor: 'white', borderRadius: 10, alignSelf: 'center', maxHeight: 300, paddingVertical: 10 }}>
          <FlatList
              ...
              return (
                <TouchableOpacity
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 8 }}>
                    <View style={{ borderRadius: 1, borderWidth: 1, borderColor: (filtered.length == 0 ? '#CCCCCC' : '#1C1C1C'), width: 18, height: 18, justifyContent: 'center', alignItems: 'center' }}>
                      {filtered.length > 0 && <AntDesign name={'check'} size={10} color={filtered.length == 0 ? '#CCCCCC' : '#1C1C1C'} />}
                    </View>

                    <Text style={{ color: '#1C1C1C', paddingVertical: 4, fontSize: 12, textAlign: 'center', marginLeft: 10 }}>{item.name}</Text>
                  </View>
  ...
  _renderDiscounts() {
    return (
      <View style={{ backgroundColor: 'transparent', width: '100%', height: '100%', position: 'absolute', justifyContent: 'center', alignItems: 'center' }}>
        <TouchableOpacity style={{ backgroundColor: 'black', opacity: 0.5, width: '100%', height: '100%', position: 'absolute' }} onPress={() => { this.setState({ showDiscountPopup: false }) }} />
        <View style={{ backgroundColor: 'white', borderRadius: 10, alignSelf: 'center', maxHeight: 300, paddingVertical: 10 }}>
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                <View style={{ flexDirection: 'row' }}>
                  <Text style={{ color: '#808080', paddingVertical: 4, fontSize: 12 }}>{(item.discountType == 'FIX_AMOUNT' ? 'Fixed' : 'Percentage') + ' -'}</Text>
                  <Text style={{ color: '#808080', paddingVertical: 4, fontSize: 12 }}>{item.discountValue + (item.discountType !== 'FIX_AMOUNT' ? '%' : '')}</Text>
  ...
  */
  ...
  _renderTwoFieldsTextInput(field1, field1Value, field2, field2Value, icon1, icon2, keyboardType1, keyboardType2, editable1, editable2) {
    return (
      ...
      <View style={{ backgroundColor: 'transparent', width: '100%', height: '100%', justifyContent: 'flex-end' }}>
        <View style={{ backgroundColor: 'white', width: '100%' }}>
          ...
          */}
          ...
          {this._renderTwoFieldsTextInput('Quantity', String(this.state.editItemDetails.quantityText), 'Unit', String(this.state.editItemDetails.unitText), 'Product', 'Product', 'number-pad', 'default', this.props.itemDetails.stock ? true : false, true)}
          {/*
            Render Rate & Amount 
          */}
          {this._renderTwoFieldsTextInput('Rate', String(this.state.editItemDetails.rateText), 'Amount', String(this.state.editItemDetails.amountText), 'Product', 'Product', 'decimal-pad', 'decimal-pad', true, false)}
          ...
          */}
          ...
          <TouchableOpacity
            ...
            style={{ marginHorizontal: 16, backgroundColor: '#5773FF', height: 50, borderRadius: 25, justifyContent: 'center', alignItems: 'center', marginTop: 15 }}>
  ...
  _renderBottomItemSheetDiscountRow() {
    return (
      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        <TouchableOpacity style={{ marginHorizontal: 16, paddingVertical: 10, flex: 1 }} onPress={() => this.setState({ showDiscountPopup: true })}>
          <View style={{ flexDirection: 'row' }}>
            <Icon name={'path-7'} size={12} />
            <Text style={{ marginLeft: 10 }} >Discount</Text>
          </View>
          <Text style={style.bottomSheetSelectTaxText} >{this.state.editItemDetails.discountType ? this.state.editItemDetails.discountType : 'Select Discount'}</Text>
