...
class MoreComponent extends React.Component<MoreComponentProp, MoreComponentState> {
  ...
  getInitails(name){
    ...
    if (allWords.length > 2){
      const twoLaterWord = allWords[0] + ' '+ allWords[allWords.length - 1];
       return twoLaterWord.split(" ").map((n)=>n[0]).join("")
    }
    else{
  ...
  render() {
    ...
    return (
      <ScrollView>
        <TouchableOpacity style={style.companyView}  onPress={() =>{
        ...
        <TouchableOpacity style={style.branchView}  onPress={() =>{
          navigation.navigate('BranchChange', {branches: this.props.branchList, activeBranch: this.state.activeBranch})
        }}>
          <Icon name ={'report'} size={20} style={style.leftView} color = {colors.PRIMARY_BASIC}  />
          <View style={{flexDirection: 'row', justifyContent: 'space-between', flex: 1}}>
            <Text style={style.companyNameText}>{activeBranchName.length > 0 ? 'Switch Branch (' + activeBranchName +')' : 'Switch Branch'}</Text>
