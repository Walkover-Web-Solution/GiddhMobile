...
class PurchaseItemEdit extends Component {
  ...
  calculateDiscountedAmount(itemDetails) {
    ...
    if (itemDetails) {
      itemDetails.discountValueText = totalDiscount;
    }
  ...
  buildMergedLineForTax(editItemDetails) {
    ...
    const computedDiscount = this.calculateDiscountedAmountToDisplayTotalAmount(e);
    const storedDiscount = Number(
      e.discountValue != null && e.discountValue !== ''
        ? e.discountValue
  ...
  calculateFinalAmount(editItemDetails) {
    ...
    const amt = this.getLineQtyForItem(editItemDetails) * this.getLineRateForItem(editItemDetails);
    let finalAmt = amt;
                  ...
                  <View style={{ marginLeft: 10 }}>
                    ...
                    <View style={{ flexDirection: 'row' }}>
                      <Text style={[{ color: '#808080', paddingTop: 4, fontSize: 12, fontFamily: FONT_FAMILY.regular }]}>
                        {(item.discountType == 'FIX_AMOUNT' ? this.props.t('purchaseItemEdit.fixed') : this.props.t('purchaseItemEdit.percentage')) + ' -'}
                      </Text>
                      <Text style={[{ color: '#808080', paddingTop: 4, fontSize: 12, fontFamily: FONT_FAMILY.regular }]}>
  ...
  _renderFinalTotal() {
    return (
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', margin: 16 }}>
        <Text>{this.props.t('purchaseBill.totalAmount')}</Text>
        ...
        <View style={{ marginHorizontal: 16, flex: 1, paddingVertical: 10 }}>
          <TouchableOpacity
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Text style={{ marginLeft: 10, color: this.props.taxArray.length > 0 ? 'black' : '#808080' }}>{this.props.t('purchaseBill.tax')}</Text>
            </View>
